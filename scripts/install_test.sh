...
test_help_flag() {
    ...
    echo "$output" | grep -q "copilot-cli" || { echo "Help output missing 'copilot-cli'"; return 1; }
...
test_opencode_commands() {
    ...
    assert_file_exists "$commands_dir/sdd-spec.md" || return 1
    assert_file_exists "$commands_dir/sdd-design.md" || return 1
    assert_file_exists "$commands_dir/sdd-tasks.md" || return 1
    local count
    count=$(find "$commands_dir" -name "sdd-*.md" | wc -l | tr -d ' ')
    assert_eq "12" "$count" "Expected exactly 12 OpenCode commands"
}
...
# ============================================================================
# Tests — GitHub Copilot CLI (project-local .github/)
# ============================================================================

test_install_copilot_cli() {
    local project="$TEST_TMPDIR/copilot-cli-project"
    mkdir -p "$project"
    (cd "$project" && bash "$INSTALL_SCRIPT" --agent copilot-cli > /dev/null 2>&1)
    assert_all_skills_installed "$project/.github/skills"
}

test_copilot_cli_skill_count() {
    local project="$TEST_TMPDIR/copilot-cli-project"
    mkdir -p "$project"
    (cd "$project" && bash "$INSTALL_SCRIPT" --agent copilot-cli > /dev/null 2>&1)
    local count
    count=$(find "$project/.github/skills" -name "SKILL.md" | wc -l | tr -d ' ')
    assert_eq "13" "$count" "Expected exactly 13 skills for GitHub Copilot CLI"
}

test_copilot_cli_engram_instructions() {
    local project="$TEST_TMPDIR/copilot-cli-project"
    mkdir -p "$project"
    (cd "$project" && bash "$INSTALL_SCRIPT" --agent copilot-cli > /dev/null 2>&1)
    assert_file_exists "$project/.github/instructions/engram.instructions.md" || return 1
    assert_file_not_empty "$project/.github/instructions/engram.instructions.md" || return 1
}
...
test_all_global_total_skill_count() {
    bash "$INSTALL_SCRIPT" --agent all-global > /dev/null 2>&1
    # 6 targets × 13 skills = 78 SKILL.md files
    local total=0
    for dir in \
        "$HOME/.claude/skills" \
    ...
    assert_eq "78" "$total" "Expected 78 total SKILL.md files across all targets"
}
...
test_all_global_opencode_commands() {
    ...
    local count
    count=$(find "$commands_dir" -name "sdd-*.md" | wc -l | tr -d ' ')
    assert_eq "12" "$count" "Expected 12 OpenCode commands with all-global"
}
...
test_idempotent_opencode() {
    ...
    assert_eq "12" "$cmd_count" "Expected exactly 12 commands after double install"
}
...
run_test "Installs 12 command files" test_opencode_commands
echo ""
...
run_test "Installs all 13 skills to .github/skills/" test_install_vscode
run_test "Exactly 13 SKILL.md files" test_vscode_skill_count
echo ""

echo -e "${BOLD}GitHub Copilot CLI (project-local)${NC}"
run_test "Installs all 13 skills to .github/skills/" test_install_copilot_cli
run_test "Exactly 13 SKILL.md files" test_copilot_cli_skill_count
run_test "Installs engram.instructions.md to .github/instructions/" test_copilot_cli_engram_instructions
echo ""
...
run_test "78 total SKILL.md files (6×13)" test_all_global_total_skill_count
